...
function buildPrompt(productData) {
  ...
  // แก้ข้อ 2+3: รับ field ให้ตรงกับที่ ProductForm ส่งมาจริงๆ
  // เอา videoStyle / duration / aiTarget ใส่ใน prompt ด้วย
  const { productName, productDetails, videoStyle, duration, aiTarget } = productData
...
ความยาววิดีโอ: ${duration} วินาที
AI ที่จะนำ Prompt ไปใช้: ${aiTarget}

โปรดสร้างครบทุกหัวข้อต่อไปนี้ โดยใส่หัวข้อนำทุกส่วน:

Hook: (ประโยคเปิดดึงดูดความสนใจ 1-2 ประโยค)

Sales Hook: (ประโยคขายแบบกดดันเวลาหรือกระตุ้นให้ซื้อทันที 3-5 ตัวเลือก เน้นให้คนที่เลื่อน Feed ต้องหยุดดู เช่น "วันนี้โปรแรง ลดแหลก รีบกดสั่ง!", "สต็อกมีจำนวนจำกัด!", "ส่งฟรีวันนี้วันเดียว!" ใช้ภาษาสั้น กระชับ มีพลัง ดูเร่งด่วน)

CTA: (Call-to-Action 3-5 ตัวเลือก สำหรับปิดการขายในคอมเมนต์ / inbox / Story เช่น "กดลิงก์ในไบโอได้เลย 👆", "DM มาหาเราได้เลยนะคะ 💬", "จำกัดออเดอร์ รีบเลย!")

Script: (สคริปต์พูดสินค้าสั้นๆ 3-5 ประโยค เหมาะกับ TikTok/Reels ปรับให้เข้ากับสไตล์ ${videoStyle})

Video Prompt: (prompt ภาษาอังกฤษสำหรับ ${aiTarget} สร้างวิดีโอสินค้า สไตล์ ${videoStyle} ความยาว ${duration} วินาที ระบุ: scene, camera movement, lighting, mood, color grading ให้ละเอียด พร้อมใช้งานได้เลย)
...
เวลาโพสต์: (แนะนำเวลาและวันที่เหมาะสมสำหรับสินค้านี้)
...
ยกเว้นหัวข้อ Video Prompt และ CHARACTER_PROMPT ให้เขียนเป็นภาษาอังกฤษเท่านั้น
`.trim()
...
export async function generateContent(productData) {
  ...
  // ── Validate input ────────────────────────────────────
  // แก้ไข: ใช้ fields จริงที่ ProductForm ส่งมา (productName, videoStyle, duration, aiTarget)
  const requiredFields = ['productName', 'videoStyle', 'duration', 'aiTarget']
  ...
  for (let i = 0; i < MODELS.length; i++) {
    ...
    try {
      ...
      // แยก CHARACTER_PROMPT ออกจาก content หลัก
      console.log('[gemini.js][generateContent] กำลัง parse CHARACTER_PROMPT...')
      const { mainContent, characterContent } = parseCharacterPrompt(text)
      console.log(`[gemini.js][generateContent] parse สำเร็จ — mainContent: ${mainContent.length} | characterContent: ${characterContent?.length ?? 0}`)
